...
public class LongOfcHand2 implements OfcHand {
	@VisibleForTesting
	...
	protected static final long MASK_FOULED = 1L << (OFF_BACK + 4*5+3);
    ...
    protected LongOfcHand2(long board) {
        this.board = board;
    }   
	...
	@VisibleForTesting
	long getBackMask() {
		return getBackMask(this.board);
	}
	static long getBackMask(long board) {
	...
	@VisibleForTesting
	long getMiddleMask() {
		return getMiddleMask(this.board);
	}
	static long getMiddleMask(long board) {
		return (board >>> OFF_MIDDLE) & MASK_MIDDLE;
	}
	
	@VisibleForTesting
	long getFrontMask() {
		return getFrontMask(this.board);
	}
	static long getFrontMask(long board) {
		return (board >>> OFF_FRONT) & MASK_FRONT;
	}

	private long withBack(OfcCard card) {
		if (isBackComplete()) {
			throw new IllegalStateException("Back already full");
		}
		long backMask = addCardToSubhand(getBackMask(), card);
		long newBoard = board & (~ (MASK_BACK << OFF_BACK));
		newBoard |= backMask << OFF_BACK;
		return newBoard;
	}
	@Override
	...
	private long withMiddle(OfcCard card) {
		if (isMiddleComplete()) {
			throw new IllegalStateException("Middle already full");
		}
		long middleMask = addCardToSubhand(getMiddleMask(), card);
		long newBoard = board & (~ (MASK_MIDDLE << OFF_MIDDLE));
		newBoard |= middleMask << OFF_MIDDLE;
		return newBoard;
	}
	
	@Override
	public void addMiddle(OfcCard card) {
		board = withMiddle(card);
	}
	private long withFront(OfcCard card) {
		if (isFrontComplete()) {
			throw new IllegalStateException("Front already full");
		}
		long frontMask = addCardToSubhand(getFrontMask(), card);
		long newBoard = board & (~ (MASK_FRONT << OFF_FRONT));
		newBoard |= frontMask << OFF_FRONT;
		return newBoard;
	}
	
	@Override
	public void addFront(OfcCard card) {
		board = withFront(card);
	}
	protected boolean isFrontComplete() {
		return isFrontComplete(board);
	}
	protected static boolean isFrontComplete(long board) {
		return 0 != (board & (MASK_RANK << (OFF_FRONT + OFF_FIRST_RANK +
											OFF_RANK * (OfcHand.FRONT_SIZE - 1))));
	}
	protected boolean isMiddleComplete() {
		return isMiddleComplete(board);
	}
	protected static boolean isMiddleComplete(long board) {
		return 0 != (board & (MASK_RANK << (OFF_MIDDLE + OFF_FIRST_RANK +
											OFF_RANK * (OfcHand.MIDDLE_SIZE - 1))));
	}
	protected boolean isBackComplete() {
		return isBackComplete(board);
	}
	protected static boolean isBackComplete(long board) {
		return 0 != (board & (MASK_RANK << (OFF_BACK + OFF_FIRST_RANK +
											OFF_RANK * (OfcHand.BACK_SIZE - 1))));
	}
	
	@Override
	public boolean isComplete() {
		return isFrontComplete(board) && isMiddleComplete(board) && isBackComplete(board);
	}


	@Override
	public long getFrontRank() {
		return getFrontRank(this.board);
	}

	public static long getFrontRank(long board) {
		long frontMask = getFrontMask(board);
		...
		if (!isFrontComplete(board)) {
			throw new IllegalStateException("Front not complete");
		}
		int[] ranks = getRanks(frontMask, OfcHand.FRONT_SIZE);
		long frontValue = StupidEval.eval3(ranks);
	...
	public long getMiddleRank() {
		return getMiddleRank(this.board);
	}
	static public long getMiddleRank(long board) {
		long middleMask = getMiddleMask(board);
		int handInt = (int) (middleMask >> OFF_UNSUITED);
		long cached = EVAL_CACHE[handInt];
		if (cached > 0) {
			return cached;
		}
		if (!isMiddleComplete(board)) {
			throw new IllegalStateException("Middle not complete");
		}
		int[] ranks = getRanks(middleMask, OfcHand.MIDDLE_SIZE);
		boolean isFlush = isSuited(middleMask);
		long middleValue = StupidEval.eval(ranks, isFlush);
	...
	public long getBackRank() {
		return getBackRank(this.board);
	}
	static public long getBackRank(long board) {
		long backMask = getBackMask(board);
		int handInt = (int) (backMask >> OFF_UNSUITED);
		long cached = EVAL_CACHE[handInt];
		if (cached > 0) {
			return cached;
		}
		if (!isBackComplete(board)) {
			throw new IllegalStateException("Back not complete");
		}
		int[] ranks = getRanks(backMask, OfcHand.BACK_SIZE);
		boolean isFlush = isSuited(backMask);
		long backValue = StupidEval.eval(ranks, isFlush);
	...
	*/
	...
	public Set<OfcHand> generateHands(OfcCard card) {
		...
		if (getBackSize() < OfcHand.BACK_SIZE) {
			LongOfcHand2 copy = this.copy();
			copy.addBack(card);
			hands.add(copy);
		}
		if (getMiddleSize() < OfcHand.MIDDLE_SIZE) {
			LongOfcHand2 copy = this.copy();
			copy.addMiddle(card);
			hands.add(copy);
		}
		if (getFrontSize() < OfcHand.FRONT_SIZE) {
			LongOfcHand2 copy = this.copy();
			copy.addFront(card);
			hands.add(copy);
		}
	...
	public long generateOnlyHand(OfcCard card) {
		long newBoard;
		if (!isFrontComplete()) {
			newBoard = withFront(card);
		} else if (!isMiddleComplete()) {
			newBoard = withMiddle(card);
		} else if (!isBackComplete()) {
			newBoard = withBack(card);
		}  else {
			throw new IllegalArgumentException("Hand full!");
		}
		long middle = getMiddleRank(newBoard);
		if ((getBackRank(newBoard) < middle) ||
			(middle < getFrontRank(newBoard))) {
			newBoard |= MASK_FOULED;
		}
		return newBoard;
	}
